...
button_callback (Widget * w, widget_msg_t msg, int parm)
{
    ...
    case WIDGET_HOTKEY:
        ...
        {
            if (g_ascii_tolower ((gchar) b->text.hotkey[0]) == g_ascii_tolower ((gchar) parm))
            {
                button_callback (w, WIDGET_KEY, ' ');
                return MSG_HANDLED;
            }
...
WButton *
button_new (int y, int x, int action, int flags, const char *text, bcback callback)
{
...
static cb_ret_t
dlg_hist_callback (Dlg_head * h, Widget * sender, dlg_msg_t msg, int parm, void *data)
{
    switch (msg)
    {
    case DLG_RESIZE:
