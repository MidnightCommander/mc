 ...
 */
static key_def *
create_sequence (const char *seq, size_t len, int code, int action)
{
    key_def *base, *attach;
 ...
 *
 * len may contain seq's actual length, in which case seq might contain an embedded NUL.
 * Otherwise, if len is negative then it's a standard NUL-terminated C string.
 */

gboolean
define_sequence (int code, const char *seq, ssize_t len, int action)
{
    size_t actual_len;
    key_def *base;
    ...
    if (len < 0)
        ...
        actual_len = (gsize) len;

    if (actual_len > SEQ_BUFFER_LEN - 1)
        return FALSE;

    for (base = keys; base != NULL && actual_len != 0;)
        if (*seq == base->ch)
        {
            if (base->child == NULL)
            {
                if (actual_len > 1)
                    base->child = create_sequence (seq + 1, actual_len - 1, code, action);
                else
                {
        ...
        else
        {
            ...
            {
                base->next = create_sequence (seq, actual_len, code, action);
                return TRUE;
            }
        }
    ...
    keys = create_sequence (seq, actual_len, code, action);
    return TRUE;
}
