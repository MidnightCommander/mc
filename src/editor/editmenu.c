...
create_file_menu (void)
{
    ...
    entries = g_list_prepend (entries, menu_entry_create (_("&Open file..."), CK_Load));
    entries = g_list_prepend (entries, menu_entry_create (_("&New"), CK_New));
    entries = g_list_prepend (entries, menu_separator_create ());
    entries = g_list_prepend (entries, menu_entry_create (_("&Save"), CK_Save));
    entries = g_list_prepend (entries, menu_entry_create (_("Save &as..."), CK_Save_As));
    entries = g_list_prepend (entries, menu_separator_create ());
    entries = g_list_prepend (entries, menu_entry_create (_("&Insert file..."), CK_Insert_File));
    entries = g_list_prepend (entries, menu_entry_create (_("Cop&y to file..."), CK_Save_Block));
    entries = g_list_prepend (entries, menu_separator_create ());
    entries = g_list_prepend (entries, menu_entry_create (_("&User menu..."), CK_User_Menu));
    entries = g_list_prepend (entries, menu_separator_create ());
...
create_edit_menu (void)
{
    ...
    entries = g_list_prepend (entries, menu_separator_create ());
    entries =
        g_list_prepend (entries, menu_entry_create (_("&Toggle ins/overw"), CK_Toggle_Insert));
    entries = g_list_prepend (entries, menu_separator_create ());
    entries = g_list_prepend (entries, menu_entry_create (_("To&ggle mark"), CK_Mark));
    entries = g_list_prepend (entries, menu_entry_create (_("&Mark columns"), CK_Column_Mark));
    entries = g_list_prepend (entries, menu_entry_create (_("Mark &all"), CK_Mark_All));
    ...
    entries = g_list_prepend (entries, menu_separator_create ());
    entries = g_list_prepend (entries, menu_entry_create (_("Co&py to clipfile"), CK_XStore));
    entries = g_list_prepend (entries, menu_entry_create (_("&Cut to clipfile"), CK_XCut));
    entries = g_list_prepend (entries, menu_entry_create (_("Pa&ste from clipfile"), CK_XPaste));
    entries = g_list_prepend (entries, menu_separator_create ());
    entries = g_list_prepend (entries, menu_entry_create (_("&Beginning"), CK_Beginning_Of_Text));
    entries = g_list_prepend (entries, menu_entry_create (_("&End"), CK_End_Of_Text));
...
create_search_replace_menu (void)
{
    ...
    entries = g_list_prepend (entries, menu_entry_create (_("&Search..."), CK_Find));
    entries = g_list_prepend (entries, menu_entry_create (_("Search &again"), CK_Find_Again));
    entries = g_list_prepend (entries, menu_entry_create (_("&Replace..."), CK_Replace));
    entries = g_list_prepend (entries, menu_separator_create ());
    entries =
        g_list_prepend (entries, menu_entry_create (_("&Toggle bookmark"), CK_Toggle_Bookmark));
    entries = g_list_prepend (entries, menu_entry_create (_("&Next bookmark"), CK_Next_Bookmark));
    entries = g_list_prepend (entries, menu_entry_create (_("&Prev bookmark"), CK_Prev_Bookmark));
    entries =
        g_list_prepend (entries, menu_entry_create (_("&Flush bookmark"), CK_Flush_Bookmarks));
...
create_command_menu (void)
{
    ...
    entries =
        g_list_prepend (entries, menu_entry_create (_("&Toggle line state"), CK_Toggle_Line_State));
    entries =
    ...
    entries = g_list_prepend (entries, menu_separator_create ());
    entries =
        g_list_prepend (entries, menu_entry_create (_("&Find declaration"), CK_Find_Definition));
    entries =
        g_list_prepend (entries,
    ...
    entries =
        g_list_prepend (entries, menu_entry_create (_("&Execute macro..."), CK_Execute_Macro));
    entries = g_list_prepend (entries, menu_entry_create (_("Delete macr&o..."), CK_Delete_Macro));
    entries = g_list_prepend (entries, menu_separator_create ());
    entries =
        g_list_prepend (entries, menu_entry_create (_("'ispell' s&pell check"), CK_Pipe_Block (1)));
...
create_format_menu (void)
{
    ...
    entries =
        g_list_prepend (entries, menu_entry_create (_("Insert &literal..."), CK_Insert_Literal));
    entries = g_list_prepend (entries, menu_entry_create (_("Insert &date/time"), CK_Date));
    entries = g_list_prepend (entries, menu_separator_create ());
    entries =
        g_list_prepend (entries, menu_entry_create (_("&Format paragraph"), CK_Paragraph_Format));
    entries = g_list_prepend (entries, menu_entry_create (_("&Sort..."), CK_Sort));
    entries = g_list_prepend (entries, menu_entry_create (_("&Paste output of..."), CK_ExtCmd));
    entries =
        g_list_prepend (entries, menu_entry_create (_("&External formatter"), CK_Pipe_Block (0)));
...
create_options_menu (void)
{
    ...
    entries = g_list_prepend (entries, menu_entry_create (_("&General..."), CK_Edit_Options));
    entries = g_list_prepend (entries, menu_entry_create (_("Save &mode..."), CK_Edit_Save_Mode));
    ...
    entries = g_list_prepend (entries, menu_separator_create ());
    entries = g_list_prepend (entries, menu_entry_create (_("S&yntax file"), CK_Load_Syntax_File));
    entries = g_list_prepend (entries, menu_entry_create (_("&Menu file"), CK_Load_Menu_File));
    entries = g_list_prepend (entries, menu_separator_create ());
