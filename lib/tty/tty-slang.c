...
static int
get_maybe_wacs (mc_tty_char_t c, gboolean *alt_char)
{
    *alt_char = TRUE;
...
tty_print_char (mc_tty_char_t c)
{
    ...
    if (mc_global.utf8_display)
        ch = get_maybe_wacs (c, &alt_char);
    else
        ch = get_maybe_acs (c, &alt_char);

    if (alt_char)
        SLsmg_draw_object (SLsmg_get_row (), SLsmg_get_column (), ch);
    else
        SLsmg_write_char ((SLwchar_Type) ((unsigned int) ch));
}
