...
extfs_fill_names (struct vfs_class *me, fill_names_f func)
{
    ...
    while (a)
    {
        name = g_strconcat (a->name ? a->name : "", "#", extfs_prefixes[a->fstype], (char *) NULL);
        (*func) (name);
        g_free (name);
        a = a->next;
    }
...
#if 0
static void
extfs_free_entries (struct entry *entry)
{
    (void) entry;
    return;
}
    ...
    while (fgets (buffer, 4096, extfsd) != NULL)
    {
        ...
        if (vfs_parse_ls_lga (buffer, &hstat, &current_file_name, &current_link_name))
        {
            ...
            if (*cfn)
            {
                ...
                if (S_ISDIR (hstat.st_mode) && (!strcmp (p, ".") || !strcmp (p, "..")))
 ...
 */
static char *
extfs_get_path (struct vfs_class *me, const char *inname, struct archive **archive, int do_not_open)
{
...
static struct entry *extfs_find_entry_int (struct entry *dir, char *name,
...
static struct entry *
extfs_find_entry (struct entry *dir, char *name, int make_dirs, int make_file)
{
    struct entry *res;
...
static int
extfs_internal_stat (struct vfs_class *me, const char *path, struct stat *buf, int resolve)
{
    struct archive *archive;
    char *q;
    struct entry *entry;
...
{
    struct pseudofile *fp = (struct pseudofile *) extfs_open (me, path, O_RDONLY, 0);
    char *p;
...
extfs_init (struct vfs_class *me)
{
    ...
    while (extfs_no < MAXEXTFS && fgets (key, sizeof (key), cfg))
    {
        ...
        {                       /* Last line without newline or strlen (key) > 255 */
            c = &key[strlen (key) - 1];
        }
