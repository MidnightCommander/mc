...
START_TEST(test_path_to_str_flags)
{
    ...
    vpath = vfs_path_from_str ("/test1://user:passwd@host.name/#enc:KOI8-R/Ñ‚ÐµÑÑ‚Ð¾Ð²Ñ‹Ð¹/Ð¿ÑƒÑ‚ÑŒ");
    str_path = vfs_path_to_str_flags (vpath, 0, VPF_STRIP_PASSWORD);
    fail_unless (strcmp ("/test1://user@host.name/#enc:KOI8-R/Ñ‚ÐµÑÑ‚Ð¾Ð²Ñ‹Ð¹/Ð¿ÑƒÑ‚ÑŒ", str_path) == 0, "\nstr=%s\n", str_path);
    g_free (str_path);
    ...
    vpath = vfs_path_build_filename (mc_config_get_home_dir (), "test1://user:passwd@host.name/#enc:KOI8-R/Ñ‚ÐµÑÑ‚Ð¾Ð²Ñ‹Ð¹/Ð¿ÑƒÑ‚ÑŒ", NULL);
