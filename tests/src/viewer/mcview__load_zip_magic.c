...
create_test_file (const unsigned char *data, size_t size)
{
    ...
    if (size > 0)
    {
        MC_UNUSED ssize_t written = write (fd, data, size);
    }
...
START_TEST (test_zip_magic_file_loads_as_ds_file)
{
    ...
    tmp_path = create_test_file (zip_header, sizeof (zip_header));
    ck_assert_msg (tmp_path != NULL, "Failed to create temp file");

    // when
    result = mcview_load (&test_view, NULL, tmp_path, 0, 0, 0);

    // then -- should succeed and set DS_FILE datasource
    ck_assert_msg (result, "mcview_load should return TRUE for ZIP-magic file");
...
START_TEST (test_zip_magic_file_reload_no_crash)
{
    ...
    result1 = mcview_load (&test_view, NULL, tmp_path, 0, 0, 0);
    ck_assert_msg (result1, "First mcview_load should return TRUE");
    ck_assert_int_eq (test_view.datasource, DS_FILE);
    ...
    result2 = mcview_load (&test_view, NULL, tmp_path, 0, 0, 0);

    // then
    ck_assert_msg (result2, "Second mcview_load should return TRUE");
    ck_assert_int_eq (test_view.datasource, DS_FILE);
    ck_assert_int_eq (mcview_show_error__call_count, 0);
...
START_TEST (test_normal_file_loads_as_ds_file)
{
    ...
    tmp_path = create_test_file (text_data, sizeof (text_data) - 1);
    ck_assert_msg (tmp_path != NULL, "Failed to create temp file");
    ...
    // then
    ck_assert_msg (result, "mcview_load should return TRUE for normal file");
    ck_assert_int_eq (test_view.datasource, DS_FILE);
    ck_assert_int_eq (mcview_show_error__call_count, 0);
...
START_TEST (test_nonexistent_file_fails)
{
    ...
    // when
    result = mcview_load (&test_view, NULL, "/nonexistent/file/path", 0, 0, 0);

    // then
    ck_assert_msg (!result, "mcview_load should return FALSE for nonexistent file");
...
START_TEST (test_gzip_magic_file_loads_as_ds_file)
{
    ...
    tmp_path = create_test_file (gzip_header, sizeof (gzip_header));
    ck_assert_msg (tmp_path != NULL, "Failed to create temp file");
    ...
    // then
    ck_assert_msg (result, "mcview_load should return TRUE for gzip-magic file");
    ck_assert_int_eq (test_view.datasource, DS_FILE);
    ck_assert_int_eq (mcview_show_error__call_count, 0);
